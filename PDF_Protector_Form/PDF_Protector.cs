    ...
    public partial class PDF_Protector : Form
    {
        ...
        private void applyPasswordButton_Click(object sender, EventArgs e)
        {
            ...
            if (Directory.Exists(fullPath) && clientPasswordTextBox.Text != "")
            {
                ...
                MessageBox.Show(
                    $"There are {initialPDFcount} PDFs in the folder. Password {passwordMsg} {countPDFs} PDFs!\nIf numbers don't match, some PDFs had (different) password or were corrupted already before!",
                    "Done with PDFs!",
        ...
        private void PDF_Protector_DragDrop(object sender, DragEventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Dragged file is not a PDF!\nPlease drag a .pdf file!",
                    "Drag a PDF!",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information
                );
